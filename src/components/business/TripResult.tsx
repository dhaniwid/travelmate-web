...
export default function TripResult({data}: { data: TripResponse }) {
    ...
    const budgetConfig: Record<string, { icon: React.ReactNode, color: string, lightBg: string }> = {
        ...
        misc: {
            ...
            color: "bg-slate-600",
            lightBg: "bg-slate-50 text-slate-700"
        },
    };
            ...
            <div className="p-6 md:p-10">
                    ...
                    <TabsContent value="transport" className="space-y-10 focus-visible:outline-none">
                        ...
                        <section>
                            <h3 className="text-xl font-bold mb-6 flex items-center gap-3 text-slate-800">
                                <div className="p-2 bg-orange-100 rounded-lg"><Plane
                            ...
                            <div className="space-y-4">
                                ...
                                {(!plan?.transport_options || plan.transport_options.length === 0) && (
                                    <p className="text-slate-400 italic text-sm border-2 border-dashed rounded-xl p-8 text-center">Finding
                                        best routes...</p>
                                )}
                            </div>
                        </section>
                        ...
                        <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in zoom-in-95 duration-500">
                            ...
                            <div
                                ...
                                <div
                                    ...
                                    <div>
                                        <p className="text-blue-100 text-sm font-bold uppercase tracking-widest mb-1">Estimated
                                            Total Investment</p>
                            ...
                            {plan?.budget_breakdown ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {Object.entries(plan.budget_breakdown).map(([key, val]) => {
                                        ...
                                        return (
                                            <div key={key}
                                                 className="group bg-white p-5 rounded-2xl border border-slate-100 hover:border-blue-200 hover:shadow-xl hover:shadow-blue-50 transition-all duration-300">
                                                <div className="flex justify-between items-start mb-4">
                                                    <div
                                                        className={`p-3 rounded-xl ${config.lightBg} transition-colors`}>
                                                        {config.icon}
                                                    </div>
                                                    <div className="text-right">
                                                        <span
                                                            className="text-[10px] font-black uppercase tracking-tighter text-slate-400 block mb-1">
                                                            {key}
                                                        </span>
                                                        <span
                                                            className="text-lg font-bold text-slate-800">{formatMoney(val)}</span>
                                                    </div>
                                                </div>
                                ...
                                <div
                                    ...
                                    <p className="text-slate-500 font-medium animate-pulse">Calculating optimal
                                        allocation...</p>
                                </div>
                            )}
                ...
                {plan?.decision_notes && plan.decision_notes.length > 0 && (
                    <div className="mt-12 p-6 bg-blue-50 rounded-2xl border border-blue-100 flex gap-4 items-start">
                        ...
                        <div>
                            <h4 className="font-bold text-blue-900 text-sm uppercase tracking-wider mb-2">TravelMate AI
                                Insights</h4>
